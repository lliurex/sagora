...
contains(CONFIG, "noupcasename") {
    message(The target name is sagora instead of Jamulus.)
    TARGET = sagora
}
...
win32 {
    ...
    mingw* {
        ...
        QMAKE_LFLAGS += /DYNAMICBASE:NO # fixes crash with libjack64.dll, see https://github.com/corrados/sagora/issues/93
        LIBS += ole32.lib \
            user32.lib \
            advapi32.lib \
            winmm.lib \
    ...
    count(nosoundoption, 0) {
        ...
        contains(CONFIG, "raspisagora") {
            message(Using Jack Audio in raspisagora.sh mode.)
            LIBS += -ljack
        } else {
